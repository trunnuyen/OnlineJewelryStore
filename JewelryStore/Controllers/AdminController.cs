...
namespace JewelryStore.Controllers
{
    ...
    {
		// GET: Admin
		JewelryStoreEntities db = new JewelryStoreEntities();
		...
		public ActionResult Index()
        {
			...
			for (int i = 0; i < 12; i++)
			{
				...
				var temp = db.Order.Where(a => a.Orderdate.Value.Month == dateTimeNow.Month).Sum(s => s.Totalprice);
				if (temp == null)
				{
					temp = 0;
				}
		...
		public ActionResult Login(FormCollection collection)
		{
			...
			Admin ad = db.Admin.SingleOrDefault(n => n.Username == userName && n.Passwords == passWord);
			if (ad != null)
			{
				Session["Account"] = ad;
				Response.Cookies["usr"].Value = ad.Username;

				var name = db.Admin.SingleOrDefault(a => a.Username == ad.Username).Name;
				Response.Cookies["Name"].Value = name;

				var atar = db.Admin.SingleOrDefault(a => a.Username == ad.Username).Picture;
				if (atar == null || atar == "")
				{
					atar = "~/img/Item/avatar-default-icon.png";
				}
		...
		{
			var temp = db.Order.Where(o => o.Status == false).ToList();
		...
		public ActionResult Comfirm(long ? id)
		{
			...
			ViewBag.Date = db.Order.SingleOrDefault(a => a.ID == id).Deliverydate;
			ViewBag.id = id;
		...
		public ActionResult Productnotsold()
		{
			...
			//			  select t1;
			var results = from t1 in db.Item
						  where !(from t2 in db.Order
								  join a in db.OrderDetail on t2.ID equals a.OrderID
